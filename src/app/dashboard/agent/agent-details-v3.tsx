...
export function AgentDetailsV3({ agentId }: { agentId?: string }) {
  ...
  const loadAgent = async () => {
    ...
    try {
      ...
      // Extract V3 config data
      if (data.config && data.config.version === 3) {
        const v3Config = data.config as AgentConfigV3;
        setName(data.name || "");
        setSystemPrompt(v3Config.data.systemPrompt || "");
        setModelConfig(v3Config.data.model || DEFAULT_MODEL);
        setTools(v3Config.data.tools || []);
      } else {
        errorToast("Agent is not using V3 schema");
        router.push("/dashboard/agents");
      }
    } catch (error: any) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const configData: AgentConfigDataV3 = {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-green-400 mt-1">Agent Config v3</p>
          </div>
        </div>
        {isOwner && (
          <button
