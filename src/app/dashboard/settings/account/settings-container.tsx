...
export function SettingsContainer() {
  ...
  useEffect(() => {
    async function fetchAccount() {
      try {
        ...
        setNewsletterSubscribed(account.newsletter_subscribed);
      } catch (err) {
        errorReporter(err);
      } finally {
        setIsLoading(false);
  ...
  const handleNewsletterToggle = async () => {
    ...
    try {
      await updateAccount({ newsletter_subscribed: newValue });
      successToast(
        newValue
              ...
              <div className="flex items-center justify-between">
                ...
                {isLoading ? (
                  ...
                  <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    />
                    <div
                      className={`w-11 h-6 bg-gray-700 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-800 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600 ${
                        isUpdatingNewsletter ? "opacity-50 cursor-wait" : ""
                      }`}
                    />
                  </label>
                )}
