...
export function SesEmailAnalyticsContainer() {
  ...
  const fetchData = useCallback(async (params: ListEmailEventsParams, silent = false) => {
    ...
    try {
      const [eventsData, statsData] = await Promise.all([
        ...
        emailEventsService.getStats({ from_date: params.from_date, to_date: params.to_date }),
      ]);
  ...
  useEffect(() => {
    ...
    if (!autoRefresh) { setCountdown(AUTO_REFRESH_INTERVAL / 1000); return; }
  ...
  const handleClear = () => {
    setRecipient(""); setMessageId(""); setEventType(""); setFromDate(""); setToDate("");
    setApplied({});
    setPage(1);
    fetchData({});
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
        ...
        <StatCard label="Delivered" value={stats.delivery}
          sub={pct(stats.delivery, stats.send) ? `${pct(stats.delivery, stats.send)} of sent` : undefined}
          icon={CheckCircleIcon} color="text-green-400" bg="bg-green-900/20" border="border-green-700/40" />
        <StatCard label="Opens" value={stats.open}
          sub={pct(stats.open, stats.delivery) ? `${pct(stats.open, stats.delivery)} of delivered` : undefined}
          icon={ArrowTrendingUpIcon} color="text-purple-400" bg="bg-purple-900/20" border="border-purple-700/40" />
        <StatCard label="Bounces" value={stats.bounce}
          sub={pct(stats.bounce, stats.send) ? `${pct(stats.bounce, stats.send)} of sent` : undefined}
          icon={ExclamationTriangleIcon} color="text-red-400" bg="bg-red-900/20" border="border-red-700/40" />
      ...
      <div className="rounded-xl border border-gray-700/50 bg-gray-900/50 p-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          />
          <input type="text" placeholder="Provider Message ID"
          ...
          <div className="flex items-center gap-3">
            ...
            {!loading && (
              <span className="text-xs text-gray-500">
                {events.length.toLocaleString()} total record{events.length !== 1 ? "s" : ""}
              </span>
            )}
            {refreshing && (
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-16 text-center">
                    ...
                    <p className="text-gray-500 text-sm">No email events found.</p>
                    {Object.keys(applied).length > 0 && (
                      <button onClick={handleClear} className="mt-2 text-xs text-blue-400 hover:text-blue-300">
                        Clear filters
                      </button>
                    )}
                  </td>
                ...
                pageEvents.map((ev, idx) => (
                  <tr key={ev.id} className="hover:bg-gray-800/40 transition-colors group">
                    ...
                    <td className="px-5 py-3 text-sm text-gray-300 max-w-[180px] truncate" title={ev.primary_recipient ?? ""}>
