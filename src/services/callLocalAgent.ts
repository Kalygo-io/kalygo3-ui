import { Action } from "@/app/dashboard/local-agent/chat-session-reducer";
...
export async function callLocalAgent(
  ...
  console.log(
    "Starting Local Agent call with sessionId:",
    sessionId,
    "prompt:",
    prompt
  );
  ...
  const resp = await fetch(
    `${process.env.NEXT_PUBLIC_AI_API_URL}/api/local-agent/completion`,
    {
      method: "POST",
      headers: {
  ...
  try {
    while (true) {
      ...
      if (abortController?.signal.aborted) {
        console.log("Local Agent request cancelled");
        break;
      }
    ...
    if (error instanceof Error && error.name === "AbortError") {
      console.log("Local Agent request aborted");
      throw error;
    }
