...
export async function callRemoteAgent(
  ...
  console.log(
    "Starting Remote Agent call with sessionId:",
    sessionId,
    "prompt:",
    prompt
  );
  ...
  const resp = await fetch(
    `${process.env.NEXT_PUBLIC_AI_API_URL}/api/remote-agent/completion`,
    {
      method: "POST",
      headers: {
  ...
  try {
    while (true) {
      ...
      if (abortController?.signal.aborted) {
        console.log("Remote Agent request cancelled");
        break;
      }
    ...
    if (error instanceof Error && error.name === "AbortError") {
      console.log("Remote Agent request aborted");
      throw error;
    }
