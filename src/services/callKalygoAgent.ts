...
export async function callKalygoAgent(
  sessionId: string,
  prompt: string,
  ...
  console.log(
    "Starting Kalygo Agent call with sessionId:",
    sessionId,
    "prompt:",
    prompt
  );
  ...
  const resp = await fetch(
    `${process.env.NEXT_PUBLIC_AI_API_URL}/api/kalygo-agent/completion`,
    {
      method: "POST",
      headers: {
  ...
  try {
    while (true) {
      ...
      if (abortController?.signal.aborted) {
        console.log("Kalygo Agent request cancelled");
        break;
      }
    ...
    if (error instanceof Error && error.name === "AbortError") {
      console.log("Kalygo Agent request aborted");
      throw error;
    }
